...
async function handler(ctx) {
    ...
    const pageNumber = ctx.req.query('page_number') ? Number.parseInt(ctx.req.query('page_number')) : 1;
    ...
    const buildItem = (e) => {
        ...
        return {
            title: String(getOne([e.target.title, e.target.question ? e.target.question.title : ''])),
